...
function EffectPI(inContext, inLanguage, inStreamDeckVersion, inPluginVersion) {
	...
	let effectSelect = `<div class="sdpi-item"><div class="sdpi-item-label" id="effect-label"></div id="effect-select"><select class="sdpi-item-value select" id="effect-select"><option id="no-effect" select="no-effect"></option></select></div>`;
	...
	function valueChanged(inEvent) {
		if (inEvent.target.select === "no-effect") {
			return;
		}
		// Save the new effect settings
		window.settings.select = inEvent.target.value;
		instance.saveSettings();
		// Inform the plugin that a new effect is set
		instance.sendToPlugin({"piEvent": "valueChanged", "settings": {"command": window.settings.command, "nanoController": window.settings.nanoController, "select": window.settings.select}});
	}
}
