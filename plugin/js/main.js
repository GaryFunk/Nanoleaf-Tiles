	...
	websocket.onmessage = function (inEvent) {
		...
		switch (event) {
			case "keyDown":
				...
				timerLPButtons.set(context, timerLP)
				break;
			case "keyUp":
				timerLP = timerLPButtons.get(context);
				if (timerLP) {
					clearTimeout(timerLP);
					timerLPButtons.delete(context)
				}
		...
		function longPress(context) {
			let timerLP = timerLPButtons.get(context);
			...
			switch (inAction[3]) {
				case "brightnessd":
				case "brightnessi":
					window.timeoutLPButtons[context] = 1;
					break;
			}
		}
	};
