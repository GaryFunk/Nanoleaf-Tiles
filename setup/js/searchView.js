				...
				if (status) {
					...
					setTimeout(function () {
						if (nanoNeedIPs.length === 0) { // No new controller found
							...
							// Fill the content area
							content = `<p>${content}</p><br><br><p style="color: yellow;">${localization["Search"]["DescriptionFound"]}</p><div class="button button-green" id="continue">${localization["Search"]["Continue"]}</div><div class="button button-blue" id="retry">${localization["Search"]["Retry"]}</div><div class="button button-red" id="exit">${localization["Search"]["Exit"]}</div>`;
